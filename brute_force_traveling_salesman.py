...
def main_route (nodes=None, instanceSize = 9, travel_mode = "driving"):
    ...
    for item in shortest_distances[0]:
        final_str  += "{lat : " + str(Nodes[str(item)][0]) + ", lng:" + str(Nodes[str(item)][1]) + "},"

    final_str = final_str[:-1] + "]"


    with open(str(km) + "km_" + travel_mode + "_output.js", mode = "w") as f:
        f.write(final_str)
...
if __name__=='__main__':
    ...
    # please add your file path to below including lattitudes and longtitudes
    filename = '.......................'

    positions = []
    id = []  # id of a locations
    lats = []  # lattitude of locations
    lons = []  # longtitude of locations
    positions = [] # lattitude, longtitude and id of locations
    places = []  # name of locations
    types = []  # types of locations like restaurant or museum
    locations = [] # lattitude and longtitude of locations
    ids = []
    ...
    #reads googlemaps distances from API
